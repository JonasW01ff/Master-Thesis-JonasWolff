...
int main() {
    ...
    vector<num> myFutOptionality((priceDate-startDate).count(),0.);
    ...
    for (date monitordate = startDate; monitordate<priceDate; monitordate= monitordate + days(1)){
        ...
        auto myf = [monitordate,idx, year0, &mydates, &myFutOptionality, &myruntime]() {

            try {
                num myYear = year0.yearsuntil(monitordate);
                pair<num,num> myres = getfutpriceNoOpt(monitordate);
                pair<num,num> myresold = getfutprice(monitordate);
                mydates[idx] = myYear;
