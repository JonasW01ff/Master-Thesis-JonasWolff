...
pair<num,num> getHWEVvolAtDate(const date &monitordate){
    ...
    return make_pair<num,num>(myHWEV.getMeanReversion(0.1), myHWEV.getVol(0.1));

}

pair<num, num> getfutprice(const date& monitorDate,  const num &yieldshift = 0) {
    vector<bond> myBonds = getBondData();
    vector<num> myPrices = getBondPriceAtDate(monitorDate);
    pair<double, double> swaptiondata = getSwaptionAtDate(monitorDate);
    // Start the timer
    ...
    termstructureForwardHermite<num> myHermite("FRG",myBondData, monitorDate);
    myHermite.setParallelShift(yieldshift);
    auto end_hermite_time = std::chrono::high_resolution_clock::now();
    shortrateHWEV<num, termstructureForwardHermite<num>> myHWEV(myHermite);
    myHWEV.setVol(100.);
    cout << "CALLIBRATING" << endl;
    ...
    for (auto myBond : myBonds){
        ...
        if (startmaturity>11.)
            inc = false;
        if (maturity<2.0 or maturity>3.25)
            inc = false;
        if (inc)
...
pair<num, num> getfutpriceNoOpt(const date& monitorDate, const num &yieldshift = 0) {
    vector<bond> myBonds = getBondData();
    vector<num> myPrices = getBondPriceAtDate(monitorDate);
    pair<double, double> swaptiondata = getSwaptionAtDate(monitorDate);
    // Start the timer
    ...
    cout << "Basket has size=" << simulBONDS.size() << endl;
    num simuldate = monitorDate.yearsuntil(fut_delivery_date);
    vector<num> priceSum(basket.size());
    vector<num> maxPrice(basket.size());
    num tempprice;
    ...
    auto calcerr = [&](const num fut){
        ...
        while (true) {
            i++;
            myHWEV.getBONDsimulSobol(CF, myTenors, simuldate, simulBONDS);
            priceSum[0] += (fut * conversionfactors[0] - simulBONDS[0]);
            for (int idx = 1; idx < basket.size(); idx++) {
                priceSum[idx] += fut * conversionfactors[idx] - simulBONDS[idx];
            }
            if (i<=1){
                for (int idx =0; idx < basket.size(); idx++)
                    test[idx] = priceSum[idx]/i;
                continue;
            }
            tempprice = abs(test[0] - priceSum[0]/i);
            test[0] = priceSum[0]/i;
            for (int idx = 1; idx <basket.size(); idx++){
                tempprice = (tempprice> abs(test[idx] - priceSum[idx]/i))?tempprice:abs(test[idx] - priceSum[idx]/i);
                test[idx] = priceSum[idx]/i;
            }
            if (tempprice < numSQRTEPS)
                break;
        }
        //cout << "IDX=" << i << "; err=" << *max_element(test.begin(), test.end()) << "; fut=" << fut << endl;
        return *max_element(test.begin(), test.end());
    };
    ...
    cout<< "Fut No Opt Price " << futmid << " with error " << errmid <<" date="<< monitorDate <<  endl;
    return make_pair(futmid, elapsed_time);
...
int main() {
    ...
    vector<num> mydates;
    vector<num> myFutOptionality;
    vector<num> myrateshifts;
    vector<future<void>> myfuts;
    vector<num> myruntime;
    auto starttime = std::chrono::high_resolution_clock::now();
    date monitordate = priceDate;
    int idx = -1;
    for (num rateshift = -.1;rateshift<.1;rateshift+=.001){
        idx++;
        myrateshifts.push_back(rateshift);
        mydates.push_back(0);
        myFutOptionality.push_back(0.);
        myruntime.push_back(0);
        auto myf = [monitordate,idx, year0, rateshift, &mydates, &myFutOptionality, &myruntime]() {

            try {
                num myYear = year0.yearsuntil(monitordate);
                pair<num,num> myres = getfutpriceNoOpt(monitordate, rateshift);
                pair<num,num> myresold = getfutprice(monitordate, rateshift);
                mydates[idx] = myYear;
                myFutOptionality[idx] = myres.first - myresold.first;
    ...
    auto vit = std::remove(myFutOptionality.begin(), myFutOptionality.end(),0.);
    mydates.resize(distance(myFutOptionality.begin(),vit));


    auto endtime = std::chrono::high_resolution_clock::now();

    plot(myrateshifts,myFutOptionality);
    //ylim({125.,140.});
    ylabel("€");
    xlabel("year");
    title("Hull-White Extended Vasicek Bund-Future Price Optionality");
    ...
    if (is_same_v<num,double>)
        save("HWEVpriceOverTimeNoOptDouble.png");
    else
        save("HWEVpriceOverTimeNoOptSingle.png");
    ::matplot::cla();
